...
int main(int argc, char const *argv[])
{
	...
	vector<vector<long long int>> costs (n+1,vector<long long int>(n+1,-1));
	for (int i = 0; i < m; ++i)
	{
		cin>>a;
		cin>>b;
...
/////////finished grabbing datas/////////////
	...
	long long int res = DFS(allcity,0,1,-1,n,visited);
	cout<<res;
	return 0;

}

long long int DFS(vector<vector<long long int>> &allcity, vector<vector<long long int>> costs,int a,long long int minpass, int n, unordered_set<int>visited) {
	long long int cur = 0;
	long long int min = -1;
	visited.insert(a);
	if(a==n)
